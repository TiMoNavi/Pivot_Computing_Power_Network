...
function Add({ id, from, stepKey, back, ...props }) {
// const Add: FC<Props> = ({ id, from, stepKey, back, ...props }) => {
  ...
  const [selectedDataset, setSelectedDataset] = useState(id ? Number(id) : 1)
  ...
  const [publicDatasets, setSets] = useState<publicDataset[]>([{
    id:1,
    name:"Tiny-Imagenet",
    keywordCount:200,
    source:"public",
  ...
  useEffect(() => {
    const fetch = async ()=>{ 
    const res = await getpublicdataset()
